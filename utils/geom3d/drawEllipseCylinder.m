...
hSurf = patch(surf2patch(x2, y2, z2), varargin{:});
% n = length(dphi);
% hSurf = mesh('Faces',repmat([1 2 4 3],n,1)+repmat(2*(0:n-1)',1,4),'Vertices',[x2(:) y2(:) z2(:)]);
...
if closed
    ...
    fcx = nan(size(hSurf.Faces,1)+2,size(pts,1)/2);
    fcx(1:size(hSurf.Faces,1),1:size(hSurf.Faces,2)) = hSurf.Faces;
    fcx(end-1,:) = 1:2:size(pts,1);
    fcx(end,:) = 2:2:size(pts,1);
    hSurf.Faces = fcx;
%     hTop = patch(x2(1,:)', y2(1,:)', z2(1,:)', color, 'edgeColor', 'none');
%     hBot = patch(x2(2,:)', y2(2,:)', z2(2,:)', color, 'edgeColor', 'none');
end
